...
const PREFERRED_VERSION = 6;
const PREFERRED_CACHE = path.join(GRANOLA_DIR, `cache-v${PREFERRED_VERSION}.json`);

const TRANSCRIPT_EXCERPT_CHARS = 4000;
const TEXT_FIELD_CHARS = 800;
const MID_TICK_QUIESCE_MS = 60_000;
...
function discoverCachePath() {
  if (process.env.LIMINAL_GRANOLA_PATH) {
    return { cachePath: process.env.LIMINAL_GRANOLA_PATH, version: null };
  }
  if (fs.existsSync(PREFERRED_CACHE)) {
    return { cachePath: PREFERRED_CACHE, version: PREFERRED_VERSION };
  }
  let best = null;
  let bestVer = -1;
  let entries;
  ...
  if (best) return { cachePath: best, version: bestVer };
  return { cachePath: PREFERRED_CACHE, version: PREFERRED_VERSION };
}

export async function ingest({ db, config, now, log }) {
  const { cachePath, version } = discoverCachePath();
  if (Number.isFinite(version) && version > PREFERRED_VERSION) {
    log?.warn?.(
      { source: "granola", discoveredVersion: version, preferred: PREFERRED_VERSION, cachePath },
      "granola_cache_newer",
    );
  }

  if (!fs.existsSync(cachePath)) {
    log?.info?.({ source: "granola", reason: "no_granola_cache", cachePath }, "granola_skip");
  ...
  try {
    ...
    log?.info?.({ source: "granola", reason: "read_error", cachePath }, "granola_skip");
  ...
  try {
    ...
    log?.info?.({ source: "granola", reason: "parse_error", cachePath }, "granola_skip");
    return { ingested: 0, skipped: "parse_error" };
  }

  const state = parsed?.cache?.state;
  if (!state || typeof state !== "object") {
    log?.info?.({ source: "granola", reason: "no_state", cachePath }, "granola_skip");
  ...
  for (const docId of Object.keys(documents)) {
    ...
    // Skip meetings updated within the last minute to avoid capturing
    // partial state from a live, in-progress recording. The next tick
    // will pick them up once they've quiesced.
    if (updatedMs > now - MID_TICK_QUIESCE_MS) continue;
